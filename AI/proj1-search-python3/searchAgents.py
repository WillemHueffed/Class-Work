...
def foodHeuristic(state, problem):
    ...
    #Original code had caching of results in problem.heuristicInfo -> this lead to not enough pruning
    #I think the issue is that the cost estimates weren't being updated for a given position after
    #a food pellet had been eaten. I.e the heuristic for each coord was only calculated once when in
    #reality it should have been updated whenever a pellet was eaten
    distances = []
    for pellet in foodGrid.asList():
        distances.append(mazeDistance(position, pellet, problem.startingGameState))
        #distances.append( ( abs(pellet[0] - position[0]) + abs(pellet[1] - position[1]) ) ) #this expands too many
    if not len(distances): #same issue as before not sure why the heurstic gets called if all the pellets have been eaten
        return 0
    else:
        return max(distances)
